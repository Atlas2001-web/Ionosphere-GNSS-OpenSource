#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Merge routine_finds_20260924p.json into PROJECTS.json and regenerate docs.
...
FINDS_PATH = ROOT / "research" / "routine_finds_20260924p.json"
LISTS = ROOT / "lists"
DOCS = ROOT / "docs"
...
def finalize_new(f: dict) -> dict:
    ...
    e = {
        ...
        "markers": f.get("markers") or ["routine-2026-09-24p"],
        "stars_approx": f.get("stars_approx", f.get("stars")),
        "source": f.get("source") or "routine-finds-20260924p",
...
def main():
    ...
    if added:
        append_notes_section(
            ...
            "## 例行检索补录（2026-09-24p）",
            (
                f"- 新增 **{len(added)}** 条（SoapyUHD/RTLSDR/Remote/Airspy/SDRPlay3、SRGI Ina-CORS、SDO/SOHO/DONKI/Helioviewer、AE/RTSW/GOES-Proton、CDDIS 轨道钟差等）\n"
                + f"- 当前条目：**{catalog['project_count']}**\n"
                + f"- 分类计数：{dict(catalog['counts_by_category'])}\n"
                + f"- 详见 `research/routine_finds_20260924p.json`\n"
            ),
        )
