#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Merge routine_finds_20260923b.json into PROJECTS.json and regenerate docs."""
...
FINDS_PATH = ROOT / "research" / "routine_finds_20260923b.json"
LISTS = ROOT / "lists"
DOCS = ROOT / "docs"
...
def finalize_new(f: dict) -> dict:
    ...
    e = {
        ...
        "markers": f.get("markers") or ["routine-2026-09-23b"],
        "stars_approx": f.get("stars_approx", f.get("stars")),
        "source": f.get("source") or "routine-finds-20260923b",
...
def main():
    ...
    notes.write_text(
        ...
        + f"\n\n## 例行检索补录（2026-09-23b）\n\n"
        + f"- 新增 **{len(added)}** 条（RINGO/M3G/EPOS-GLASS/IBGE-PPP/CSRS-PPP/OPUS-Projects/ncat-lib/BEV-APOS/TITIPy 等）\n"
        + f"- 当前条目：**{catalog['project_count']}**\n"
        + f"- 分类计数：{dict(catalog['counts_by_category'])}\n"
        + f"- 详见 `research/routine_finds_20260923b.json`\n",
        encoding="utf-8",
    )
