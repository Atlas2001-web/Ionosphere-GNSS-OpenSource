#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Merge routine_finds_20260926b.json into PROJECTS.json and regenerate docs.
...
FINDS_PATH = ROOT / "research" / "routine_finds_20260926b.json"
LISTS = ROOT / "lists"
DOCS = ROOT / "docs"
...
def finalize_new(f: dict) -> dict:
    ...
    e = {
        ...
        "markers": f.get("markers") or ["routine-2026-09-26b"],
        "stars_approx": f.get("stars_approx", f.get("stars")),
        "source": f.get("source") or "routine-finds-20260926b",
...
def main():
    ...
    if added:
        append_notes_section(
            ...
            "## 例行检索补录（2026-09-26b）",
            (
                f"- 新增 **{len(added)}** 条（USGS/NRCan/BGS 地磁数据、MACCS、THEMIS GMAG、TGO 磁力计链、RAL 测高仪；PySPEDAS、MagPy、ChaosMagPy、resolvedvelocities、MIPS、BAFIM、digisondeindices、isr-raw）\n"
                + f"- 当前条目：**{catalog['project_count']}**\n"
                + f"- 分类计数：{dict(catalog['counts_by_category'])}\n"
                + f"- 详见 `research/routine_finds_20260926b.json`\n"
            ),
        )
