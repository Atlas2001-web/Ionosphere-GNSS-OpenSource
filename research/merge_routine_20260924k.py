#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Merge routine_finds_20260924k.json into PROJECTS.json and regenerate docs.
...
FINDS_PATH = ROOT / "research" / "routine_finds_20260924k.json"
LISTS = ROOT / "lists"
DOCS = ROOT / "docs"
...
def finalize_new(f: dict) -> dict:
    ...
    e = {
        ...
        "markers": f.get("markers") or ["routine-2026-09-24k"],
        "stars_approx": f.get("stars_approx", f.get("stars")),
        "source": f.get("source") or "routine-finds-20260924k",
...
def main():
    ...
    if added:
        append_notes_section(
            ...
            "## 例行检索补录（2026-09-24k）",
            (
                f"- 新增 **{len(added)}** 条（novatel/max2771/RTKLIB-PPK/pysatNASA/swiftnav-ros2、SWS闪烁、ISGI/CDAWeb/SSCWeb/UKSSDC/SWPC/CCMC 等）\n"
                + f"- 当前条目：**{catalog['project_count']}**\n"
                + f"- 分类计数：{dict(catalog['counts_by_category'])}\n"
                + f"- 详见 `research/routine_finds_20260924k.json`\n"
            ),
        )
