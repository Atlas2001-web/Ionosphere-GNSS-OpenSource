#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Merge routine_finds_20260924s.json into PROJECTS.json and regenerate docs.
...
FINDS_PATH = ROOT / "research" / "routine_finds_20260924s.json"
LISTS = ROOT / "lists"
DOCS = ROOT / "docs"
...
def finalize_new(f: dict) -> dict:
    ...
    e = {
        ...
        "markers": f.get("markers") or ["routine-2026-09-24s"],
        "stars_approx": f.get("stars_approx", f.get("stars")),
        "source": f.get("source") or "routine-finds-20260924s",
...
def main():
    ...
    if added:
        append_notes_section(
            ...
            "## 例行检索补录（2026-09-24s）",
            (
                f"- 新增 **{len(added)}** 条（GAMIT/GLOBK、NASA GMAT、qzsl6tool、CSA AlouetteApp、NRL/HamSCI TID 工具、GTrop/TropDS/映射函数高程改正、anywhere-sdr 等）\n"
                + f"- 当前条目：**{catalog['project_count']}**\n"
                + f"- 分类计数：{dict(catalog['counts_by_category'])}\n"
                + f"- 详见 `research/routine_finds_20260924s.json`\n"
            ),
        )
