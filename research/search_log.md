...
- Merge safety: append-only; UPDATED=0; blanking vs HEAD=0
## Routine pass 2026-09-26b

- Catalog size before: **1011**; after merge: **1026** (+15)
- Finds file: `research/routine_finds_20260926b.json`
- Method: ISR / ionosonde / magnetometer angle — portals USGS Geomagnetism data, NRCan CANMOS magnetic data (FDSN), BGS geomag data service (distinct from imag-data INTERMAGNET), MACCS, THEMIS GMAG, TGO (UiT) magnetometer chain, RAL ionosondes (UKSSDC sub-page); tools PySPEDAS, MagPy, ChaosMagPy, AMISR resolvedvelocities, MIT Haystack MIPS, BAFIM (GUISDAP), digisondeindices, isr-raw; dedup vs PROJECTS.json + all `routine_finds_*.json` before deep research; `gh api` + `curl -sIL` (browser UA; all 200)
- Avoided: SuperMAG/CARISMA/FMI IMAGE/BC magnetometers/NICT ionosonde/BoM WDC/ISGI/Kyoto WDC/pynasonde already listed; JRO Madrigal (jro-db unreachable) and IGP JRO page (generic); schainpy (JRO-Peru repo stale, no license); GUISDAP (no canonical live home found; SourceForge only referenced); AUTUMN (502), MAGDAS (unreachable), Arecibo/NAIC (unreachable), DTU ground stations (404), Ebro/Dourbes/IAP ionosonde pages (404/unreachable); SimISR/visuamisr/spacecataz supermag/didbase-fetch/ReadSao (thin or stale); SpacePy (broad, custom licence)
- Merge safety: append-only; UPDATED=0; blanking vs HEAD=0
## Routine pass 2026-09-26c

- Catalog size before: **1026**; after merge: **1036** (+10)
- Finds file: `research/routine_finds_20260926c.json`
- Method: receiver protocol/decoder + smartphone raw-GNSS angle — mandated AeroRust `nmea` crate and NovAtel EDIE first, then Trimble GSOF (official ROS 2 driver, Wireshark plugin), Unicore UM980/UM982 (SparkFun Arduino lib, Python driver, ROS driver, PPK pipeline), smartphone RTK booster (Taro Suzuki) and BDS-RawScope Android raw-measurement app; dedup vs PROJECTS.json + all `routine_finds_*.json` before research; `gh api` + `curl -sIL` (all 200); licences read from LICENSE files where GitHub showed NOASSERTION
- Avoided: Septentrio sbfparser/sbfmixer/pydatalink/driver, pysbf/pysbf2, libsbp/piksi_tools/swiftnav-ros2, pyubx2/ublox-rs/ubxlib/SparkFun u-blox, GPSTest, gps-measurement-tools, android_rinex, novatel_oem7_driver already listed; TOTEM-release (all rights reserved), GNSSAug (PolyForm Noncommercial), unlicensed GnssLogger→RINEX converters (ozn159, zqmever, laith-uon, android2rinex) and um982 ROS repos without licence; morcki/android_raw2rinex, drcornflakes, daody (2018–2019 stale); bvernoux GNSSLogger (NMEA saturated); clrsrc/smartphone-rtk (1★ experimental); nr-sbf-parser (Node-RED, thin); awesome smartphone datasets list (no licence)
- Merge safety: append-only; UPDATED=0; blanking vs HEAD=0
