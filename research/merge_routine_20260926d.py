#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Merge routine_finds_20260926d.json into PROJECTS.json and regenerate docs.
...
FINDS_PATH = ROOT / "research" / "routine_finds_20260926d.json"
LISTS = ROOT / "lists"
DOCS = ROOT / "docs"
...
def finalize_new(f: dict) -> dict:
    ...
    e = {
        ...
        "markers": f.get("markers") or ["routine-2026-09-26d"],
        "stars_approx": f.get("stars_approx", f.get("stars")),
        "source": f.get("source") or "routine-finds-20260926d",
...
def main():
    ...
    for raw in finds:
        ...
        nu = norm_url(raw["url"])
        if is_retired(raw["url"]):
            print("SKIP retired", raw["name"], raw["url"])
            continue
        if nu in by_url:
    ...
    if added:
        append_notes_section(
            ...
            "## 例行检索补录（2026-09-26d）",
            (
                f"- 新增 **{len(added)}** 条（binex：nav-solutions Rust BINEX 编解码库，补齐 docs/software/binex.md 对应目录条目）\n"
                + f"- 当前条目：**{catalog['project_count']}**\n"
                + f"- 分类计数：{dict(catalog['counts_by_category'])}\n"
                + f"- 详见 `research/routine_finds_20260926d.json`\n"
            ),
        )
