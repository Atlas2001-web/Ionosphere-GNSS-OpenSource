#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Merge routine_finds_20260926c.json into PROJECTS.json and regenerate docs.
...
FINDS_PATH = ROOT / "research" / "routine_finds_20260926c.json"
LISTS = ROOT / "lists"
DOCS = ROOT / "docs"
...
def finalize_new(f: dict) -> dict:
    ...
    e = {
        ...
        "markers": f.get("markers") or ["routine-2026-09-26c"],
        "stars_approx": f.get("stars_approx", f.get("stars")),
        "source": f.get("source") or "routine-finds-20260926c",
...
def main():
    ...
    if added:
        append_notes_section(
            ...
            "## 例行检索补录（2026-09-26c）",
            (
                f"- 新增 **{len(added)}** 条（AeroRust nmea、NovAtel EDIE、Trimble GSOF ROS 驱动/Wireshark 插件、和芯星通 UM980/UM982 驱动与 PPK 流水线、SparkFun Unicore 库、smartphone-gnss-booster、BDS-RawScope）\n"
                + f"- 当前条目：**{catalog['project_count']}**\n"
                + f"- 分类计数：{dict(catalog['counts_by_category'])}\n"
                + f"- 详见 `research/routine_finds_20260926c.json`\n"
            ),
        )
