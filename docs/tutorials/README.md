...
- **公式**：课文使用 GitHub 支持的 `$...$`（行内）与 `$$...$$`（独立行）。请在 **github.com 网页**打开 `.md` 预览；纯文本编辑器会仍显示 LaTeX 源码，这是正常的。
- **图片**：`images/` 下为教学自制图，版权说明见 [`images/SOURCES.md`](./images/SOURCES.md)。不使用公众号等不明授权截图。
- **同一行少塞公式**：若一行里多个 `$...$` 且含 LaTeX 命令，GitHub 可能露源码；复杂式请用单独的 `$$` 块，单位可用 Unicode（m⁻²、TECU）。
