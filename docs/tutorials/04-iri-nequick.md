...
> **看图要点（为什么先看分层）**
> - **看什么**：对流层→平流层→中间层→热层/电离层的高度堆叠。
> - **为什么**：经验气候模型画的是「这段高度域里电子平均怎么堆」的肖像，不是卫星拍的天气照片；先认舞台，再谈 IRI/NeQuick。
> - **别误判**：分层示意不是某日实测廓线；D/E/F 层名字是高度域标签，不等于模型只输出三层常数。
...
| 它是 | 它不是 |
|---|---|
| 长期观测（测高仪、掩星、非相干散射、早期 TEC 等）统计出来的**平均长相** | 逐分钟同化「今天」观测的天气预报 |
| 用指数与坐标当钥匙，去查**系数表/经验公式** | 从第一性原理解 continuity + 动量 + 能量的物理数值模式 |
| 可重复：同一版本+同一指数 → 几乎同一肖像 | 「真值」或「事件精细结构」的替代品 |

类比加厚：经验模式像《气候志》——「七月午后这个纬度带平均气温多少」；GIM/双频 STEC 像「今天下午三点的温度计读数」；TIE-GCM 一类物理模式像「按流体方程跑出来的虚拟大气」——三条路，问题不同，真假维度不同。

**与实测 / GIM 差在哪（机制对照表）**：

| 维度 | IRI / NeQuick 族 | 测高仪 / 掩星 | GIM / 双频 STEC |
|---|---|---|---|
| 信息源头 | 历史统计系数 + 指数 | 当天（或近实时）探测 | 当天 GNSS 网观测反演 |
| 典型交付 | $N_e(h)$ 肖像（可积分 TEC） | 底部～峰区廓线（顶部常受限） | 柱总量场（峰高可辨识性弱） |
| 暴期行为 | 可系统性失灵（方法边界） | 仍是观测，但覆盖稀疏 | 仍受站网/平滑限制，但是实况路线 |
| 可重复性 | 版本+指数固定即可复现 | 依赖仪器与反演假设 | 依赖 AC/站网/产品档 |

口诀：**气候相册 vs 温度计 vs 物理仿真器**——论文里三者标签不许撕掉混贴。
...
> **看图要点（为什么蓝橙并排）**
> - **看什么**：平滑蓝线（IRI 气候肖像）与橙线（一次测高仪反演）的峰高 $h_mF2$、峰密 $N_mF2$ 差多少；顶部谁先「断」。
> - **为什么**：一眼区分「平均相册」与「当天探测」——差几个 TECU 或几十 km 峰高，常常是气候 vs 天气，不是程序写错。
> - **别误判**：测高仪往往到不了完整顶部；橙线断处 ≠ 电子突然为零；蓝线光滑 ≠ 更「物理真」。
> 图源：自制 CC0。

![昼侧电子密度高度剖面示意](./images/fig-ne-profile-layers.png)

> **看图要点（为什么看分层廓线）**
> - **看什么**：D/E/F 层在高度上的密度堆叠，峰区大概落在哪一段。
> - **为什么**：IRI 的核心交付是整条（或分段）$N_e(h)$；TEC 只是积分后果——先认「雾层堆在哪」，再谈柱高。
> - **别误判**：示意层名不是三个独立开关；改 F2 峰往往比改 D 层更能拧动整柱 TEC。
> 图源：自制示意。

![Chapman 型 Ne(h) 廓线](./images/fig-chapman-ne.png)

> **看图要点（为什么先学 Chapman）**
> - **看什么**：「峰在中间、上下衰减」的经典形状；峰密、峰高、厚度三旋钮如何改变胖瘦。
> - **为什么**：真实 IRI 比单峰 Chapman 更复杂，但课堂心智模型仍是这三旋钮——后面公式台会反复用。
> - **别误判**：Chapman 教学曲线 ≠ IRI 输出文件逐点相等；勿把峰密度直接叫 TEC。
> 图源：自制计算示意。
...
- 社区对照基准：论文写「与 IRI-某版符合」时，对方知道你在比哪本标准肖像；  
- 参数丰富：不止 TEC，还常给 $N_mF2$、$h_mF2$、多层形状选项等；  
- 教学与算法基准：固定版本 + 固定指数 → 可重复实验。

**IRI 不擅长什么**：超级风暴精细分区、小尺度泡/TID 锋面、需要「今天观测真值」的事件表征——这些是方法边界，不是你没调好开关。
...
> **看图要点（为什么日夜并排）**
> - **看什么**：白天抬升、夜间回落、低纬驼峰——气候肖像里最稳的「平均长相」。
> - **为什么**：模型旋钮（地方时、季节、F10.7）主要在调这类大尺度结构的**幅度与相位**；先认平均长相，暴期失真才有对照。
> - **别误判**：光滑驼峰 ≠ 捕捉了某夜等离子体泡；日夜差是气候日变化，不是磁暴证据。
> 图源：自制示意。
...
> **看图要点（为什么看指数时间线）**
> - **看什么**：Kp/Dst 等窗口何时抬升、主相/恢复相大概落在哪。
> - **为什么**：地磁指数是「翻哪一本平均相册」的钥匙之一；把安静默认指数塞进暴期仿真 = 用错相册，残差法庭应先判 **β（旋钮事故）**。
> - **别误判**：指数抬升 ≠ 模型会自动画出观测锋面；有暴期修正开关也不等于同化了这次事件。
> 图源：自制示意。
...
| | IRI | NeQuick 族（含 NeQuick2） |
|---|---|---|
| **课堂标签** | 参考标准肖像、社区对照基准 | 传播与路径积分友好的经验廓线工具 |
| **更常被问** | 「与 IRI-某版比如何？」 | 「沿这条链路 TEC/延迟多少？」 |
| **交付重心** | 多层 $N_e(h)$、峰参、选项开关多 | 便于任意射线积分的廓线构造 |
| **驱动习惯** | F10.7、Ap 等 + 公共指数文件 | Az（或由 F10.7/广播映射）缩放电离水平 |
| **单频导航** | 一般不作 Galileo 官方改正 | **NeQuick-G** 绑 Galileo 电文（见下） |

二者都能积 TEC；不要说「只有 NeQuick 才有 TEC」。差别在实现生态、输入习惯、社区用途与版本政策——**擅长舞台不同，不是谁更「真」**。
...
## 8. 解剖台 B：$N_e(h)$ → TEC（廓线参数 → 柱积分）

> 解剖台统一格式：**符号表 → 逐步推导（每步人话）→ 小数值例**。IRI/NeQuick 的核心交付是电子密度随高度的经验描述；TEC 是积分后果，不是另一个独立「测量」。
...
| 符号 | 含义 | 单位 / 备注 |
|---|---|---|
| $N_e(h)$ | 高度 $h$ 处电子数密度 | $\mathrm{m}^{-3}$（有时写 $\mathrm{el/cm^{3}}$，$1\,\mathrm{el\,cm^{-3}}=10^{6}\,\mathrm{m^{-3}}$，换算须小心） |
| $N_mF2$ | F2 层峰电子密度 | $\mathrm{m}^{-3}$；整柱的「主水龙头」 |
| $h_mF2$ | F2 层峰高 | km；雾层「堆在哪一层楼」 |
| $H$ / 半厚度等 | 形状/厚度参数（实现名各异） | km；决定峰上下胖瘦 |
| $h_{\min},h_{\max}$ | 积分下限/上限 | km；口径声明的生命线 |
| $\mathrm{d}h$ / $\mathrm{d}s$ | 竖直微元 / 射线弧长微元 | m 与 $N_e$ 的 $\mathrm{m}^{-3}$ 配套最干净 |
| VTEC / STEC | 竖直 / 斜柱总量 | TECU；$1\,\mathrm{TECU}=10^{16}\,\mathrm{el\,m^{-2}}$ |

### 8.2 逐步推导（每步人话）

1. **模型先给出** $N_e(h)$——每个高度一勺电子「雾密度」，不是直接吐一个 TEC 标签。  
2. **竖直积分**：把从 $h_{\min}$ 到 $h_{\max}$ 的密度一层层加起来 → $\mathrm{VTEC}_{\mathrm{model}}$。人话：柱高 = 各层雾的「厚度×密度」之和。  
3. **斜路径积分**：沿接收机→卫星射线弧长 $\mathrm{d}s$ 累加 → $\mathrm{STEC}_{\mathrm{model}}$。同一廓线，仰角越低通常斜柱越大（几何更长）。  
4. **峰参如何拧动整柱**：抬 $N_mF2$（峰更密）→ 主贡献区变厚重 → TEC↑；挪 $h_mF2$（峰搬家）→ 在固定积分限内「算进/漏掉」的雾量会变，且与掩星/测高仪对照时峰高本身就是判据。  
5. **厚度直觉**：半厚度变大 ≈ 峰上下更胖 → 即使 $N_m$ 不变，积分柱也可变大——「薄而密」与「厚而稀」可积出相近 TEC。  
6. **与 GIM 对照时**：GIM 直接约束的是柱；对峰高可辨识性弱——**模型有廓线、GIM 有柱，可比但不可冒充同一观测**。

### 8.3 小数值例（建立量感）

设某层厚度 $H=100\,\mathrm{km}=10^{5}\,\mathrm{m}$，层内近似均匀 $N_e=5\times10^{11}\,\mathrm{m^{-3}}$，则柱贡献
...
再练（峰参敏感度）：若只把峰区等效均匀片的 $N_e$ 从 $5\times10^{11}$ 提到 $8\times10^{11}$（其余不变），则 $\Delta\mathrm{TEC}$ 从 5 提到 8 TECU——**+3 TECU 全来自峰密**，不是「又发现了新层」。

因此课堂纪律：

1. 改 $N_mF2$ / $h_mF2$ / 厚度，TEC 会变——机理入口在廓线形状；  
2. 只比对 TEC、从不看峰高/峰密度，等于只看柱高不看「雾层堆在哪」；  
3. 积分限（是否含上层/等离子体层）声明不清 → 假残差数 TECU 级。
...
> **看图要点（为什么看峰与厚度）**
> - **看什么**：峰高 $h_m$、峰密 $N_m$、标高/半厚度 $H$ 如何决定廓线胖瘦。
> - **为什么**：积分柱是这三旋钮的后果；读模型图第一眼镜应是「峰在哪、有多尖」，不是只读一个 TEC 色块。
> - **别误判**：把峰密度直接叫 TEC；把 Chapman 教学曲线当成 IRI 输出逐点真值。
> 图源：自制示意。教学上常用 Chapman 型直觉；IRI 实现远比单峰复杂，但「峰 + 厚度」仍是读图第一眼镜。

---

## 9. 解剖台 C：驱动如何进入方程（课堂直觉链）

> 下列是**课堂直觉方程**，不是某一 Fortran 行的逐行翻译。目标：你能回答「F10.7 / Ap / 纬经时」各自拧的是肖像的哪一块。格式仍是**符号表 → 逐步推导 → 小数值例**。

### 9.1 符号表：坐标选点 + 指数选相册

| 符号 | 含义 | 单位 / 备注 |
|---|---|---|
| $(\varphi,\lambda,h,t)$ | 地理纬、经、高、世界时 | ° / km / UT；输入基本四元组 |
| $\varphi_{\mathrm{mag}}$ | 地磁纬 / 修正磁纬 | °；许多过程按磁纬组织 |
| $LT$ | 地方时 | h；由 $(t,\lambda)$ 映射 |
| $\chi$ | 太阳天顶角 | °；白天光致电离几何 |
| $F10.7$ | 10.7 cm 射电流量 | sfu；$1\,\mathrm{sfu}=10^{-22}\,\mathrm{W\,m^{-2}\,Hz^{-1}}$ |
| $Ap$ 等 | 地磁活动指数 | 无量纲档位（实现相关） |
| $N_mF2,\,h_mF2,\,\mathrm{shape}$ | 峰密、峰高、形状 | 见 §8；驱动的「出口旋钮」 |

抽象两步：① 几何–时间坐标选「相册哪一页哪个格子」；② 活动指数决定「翻哪一本平均相册」。

### 9.2 逐步推导（每步人话）

1. **输入坐标** $(\varphi,\lambda,h,t)$ → 常再映到 $\varphi_{\mathrm{mag}}$、$LT$、$\chi$。人话：先搞清「你站在磁纬哪一档、当地白天还是晚上」。  
2. **太阳活动**进入峰参：经验上白天 $N_mF2$ 与 TEC 基线随 F10.7 升高

$$
N_mF2 \;\sim\; \mathcal{F}(\varphi_{\mathrm{mag}},\,LT,\,season,\,F10.7,\ldots).
$$

3. **F10.7 偏高** → 许多地区白天峰与整柱气候期望上移（「曝光」加一档）。  
4. **填错日期或日值/多日平均用错** → 整图系统性抬高或压低——残差像「全球平移」，偏 β 而非新物理。  
5. **地磁项**（若开启）：在安静气候肖像上加统计暴时修正
...
6. **关键限制**：$\Delta N_e$ 是历史统计平均，不是本次事件同化；超级风暴可远超样本包络 → 残差巨大是预期。平静默认指数却声称「驱动了暴」→ 判 β。  
7. **纬/经/时链**（请能默写）：
...
（中文在公式外：$\mathrm{shape}$ = 半厚度等形状参数。）地理纬选结构域；经度+世界时变地方时；季节换冬夏系数；$\chi$ 是白天开关。

### 9.3 小数值例：只扫 F10.7 的气候敏感度

固定中纬某站、地方时正午、安静 Ap。课堂量级（示意，非某一版本承诺）：

| F10.7 (sfu) | 模型 VTEC 约 | 相对 F10.7=70 的增量 |
|---|---|---|
| 70 | 12 TECU | — |
| 120 | 18 TECU | +6 TECU |
| 180 | 28 TECU | +16 TECU |

读表：曝光旋钮从低年到高年，气候柱可差十余 TECU——**这是气候敏感度扫描，不是在模拟一场磁暴**。若同日 GIM≈20 TECU，则 F10.7=70 时 $R\approx+8$，F10.7=180 时 $R\approx-8$：残差符号随旋钮翻面，法庭应先疑 β。
...
> **看图要点（为什么看双峰）**
> - **看什么**：赤道异常南北驼峰的平均轮廓落在哪些磁纬带。
> - **为什么**：气候肖像可以「平均地」画出驼峰；磁纬选错格子 → 驼峰位置/符号错，残差像「东边接到西边」之前的纬度版。
> - **别误判**：平均驼峰 ≠ 捕捉某夜等离子体泡或锋面（现象细节见 21，本课不展开成文墙）。
> 图源：自制示意。
...
> 格式：**符号表 → 逐步推导 → 小数值例**。把「太阳活动如何塞进 NeQuick」与「单频机如何把模型 TEC 换成米」钉死。

### 10.1 符号表

| 符号 | 含义 | 单位 / 备注 |
|---|---|---|
| $Az$ | 有效电离水平（NeQuick 族常用太阳活动代理） | 实现相关；可由 F10.7 或 Galileo 广播映射 |
| $N_e(h;\,\ldots,Az)$ | 被 Az 缩放后的经验廓线 | $\mathrm{m}^{-3}$ |
| $\widehat{\mathrm{STEC}}$ | 模型估计的斜柱 | TECU 或 $\mathrm{el\,m^{-2}}$（须声明） |
| $I(f)$ | 一阶群延迟等效距离 | m |
| $f$ | 载波频率 | Hz（L1 量级 $\sim1.575\times10^{9}$） |
| $\mathrm{bcast}$ | Galileo 广播电离层参数 | 电文槽；NeQuick-G 专用入口 |
| $\mathrm{geom}$ | 链路几何（仰角、方位、穿刺等） | 接收机侧计算 |
...
1. **Az 升高** ≈ 「今天电离水平按更活跃的气候页来画」→ 峰与 TEC 基线上移。  
2. **Az 不是 GNSS 观测同化**——它是经验驱动旋钮，不是把双频 STEC 喂进模型。  
3. **科研 NeQuick2**：你常手填/文件给定太阳代理（如 F10.7）再映到 Az。  
4. **NeQuick-G**：Az（或等价电离参数）由 **Galileo 导航电文中的电离层参数** 在接收机侧生成——接口票据不同。  
5. **一阶群延迟**（接 01；系数以教材/标准为准，这里钉结构）：

$$
I(f)\;\approx\;\frac{40.3}{f^{2}}\,\mathrm{STEC},
$$

其中 STEC 须先换成 $\mathrm{el\,m^{-2}}$（$1\,\mathrm{TECU}=10^{16}\,\mathrm{el\,m^{-2}}$）再与 40.3 配套。  
6. 单频用户无法用双频消掉 $I$，于是用模型估 $\widehat{\mathrm{STEC}}$ 再减：
...
7. **纪律**：模型改正是折中；**不能**宣称达到双频无电离层同等水平。禁止把 NeQuick2 参数硬塞进 Galileo 电文槽还自称合规。

### 10.3 小数值例：STEC 估错 → 延迟漂多少（量级）

取 $f=1.575\times10^{9}\,\mathrm{Hz}$（L1 量级），先把系数收成「每 TECU 多少米」的课堂量级：

$$
\frac{40.3}{f^{2}}\times10^{16}
\approx 0.162\,\mathrm{m\,/\,TECU}.
$$

（验算：$f^{2}\approx2.481\times10^{18}$，$40.3/f^{2}\approx1.624\times10^{-17}$，再 $\times10^{16}\approx0.162$。）

若 NeQuick-G 估 $\widehat{\mathrm{STEC}}=20\,\mathrm{TECU}$，则 $\hat{I}\approx3.24\,\mathrm{m}$。  
若相对真值估高 **+5 TECU**，改正偏大约 **+0.81 m**——单频伪距电离层改正「多减了一截」。定性：延迟误差与 $\Delta\mathrm{STEC}$ 同号，按 $1/f^{2}$ 缩放。

![群延迟随频率变化](./images/fig-iono-delay-vs-freq.png)

> **看图要点（为什么画延迟–频率）**
> - **看什么**：同一 STEC 下，频率越低延迟越大的 $1/f^{2}$ 曲线。
> - **为什么**：单频改正猜的是「这一刀有多长」；双频测的是「两刀之差」——量级故事不同，不能把 NeQuick-G 输出冒充双频实测。
> - **别误判**：曲线光滑 ≠ 暴期改正仍然准；估错 STEC 会按 $1/f^{2}$ 放大成米级漂。
> 图源：自制计算示意。
...
> 格式：**符号表 → 逐步推导 → 小数值例**。残差是「当天的脸减平均肖像」——先审口径与旋钮，再谈物理。

![风暴−安静 / 残差示意](./images/fig-storm-quiet-residual.png)

> **看图要点（为什么对比两种差值）**
> - **看什么**：03 课「风暴−安静」实况差 vs 本节「实况−气候」模型残差，色块空间型可以很像，问题定义不同。
> - **为什么**：两者都有用；偷换基线会把「相对近期安静实况」写成「相对 IRI 气候」或反过来。
> - **别误判**：残差大 ≠ 自动等于新物理；残差小 ≠ 无天气（两边都可能被平滑）。
> 图源：自制示意。
...
| 符号 | 含义 | 单位 / 备注 |
|---|---|---|
| $X_{\mathrm{data}}$ | 实况侧场（GIM VTEC、自建映射 VTEC、或同射线 STEC） | TECU（须与模型同几何） |
| $X_{\mathrm{model}}$ | 气候侧场（IRI/NeQuick 积分或格网） | TECU；版本+指数必须可追溯 |
| $R$ | 残差 = 实况 − 气候 | TECU；符号：正 = 实况高于气候肖像 |
| γ / β / α | 法庭三档：口径 / 旋钮·失灵 / 物理 | 顺序强制，不可跳级 |

### 11.2 逐步推导（每步人话）

1. **对齐**：同一世界时（或地方时）切片；同为 VTEC 或同为同一射线 STEC；积分限尽量声明。  
2. **相减**：$R=X_{\mathrm{data}}-X_{\mathrm{model}}$——人话：今天的脸减平均肖像。  
3. **先 γ**：斜减竖、时标/经度乱、积分口径不一、单频延迟当 VTEC → 假残差，停。  
4. **再 β**：指数日期错、缺公共文件、NeQuick2↔G 混用、残差随 F10.7 整体滑却不随 Dst 走 → 旋钮/失灵，停。  
5. **最后 α**：与主相/恢复相对齐；空间型像正负暴/EIA 变形；换 GIM/换质控站仍同号；指数版本已核对。  
6. **天花板检验**：暴期模型绝对场仍「平静漂亮」；$|R|$ 巨大；拧 F10.7/Ap 造不出观测锋面 → 叙事是「气候达到天花板」，不是「模型表征了精细暴结构」。
...
### 11.3 小数值例：安静偏置 vs 暴期天花板

| 场景 | $X_{\mathrm{data}}$ | $X_{\mathrm{model}}$ | $R$ | 倾向 |
|---|---|---|---|---|
| 安静日、指数已核 | 25 TECU | 18 TECU | +7 TECU | 先排 γ/β；数 TECU 级常见，未必是 bug |
| 主相日、模型仍漂亮 | 40 TECU | 22 TECU | +18 TECU | 天花板候选：残差度量偏离，非模型「表征」风暴 |
| 故意 F10.7 错一天 | 20 TECU | 28 TECU | −8 TECU | β：大尺度偏置，禁写成 TID |
...
现象剧本（正负暴、泡、TID、耀斑/日食）见 19–23，本课只留判据，不展开成文墙。
...
### 12.0 一眼区分：气候态 vs 暴时失真

合上公式，先练「监控录像认人」。下表是值班时最常用的对照——**左边像相册，右边像事故现场**：

| 一眼看什么 | 气候态（模型正常） | 暴时失真 / 天花板 |
|---|---|---|
| 绝对场光滑度 | 大尺度光滑，少细碎锋面 | 模型仍光滑漂亮；GIM/STEC 已大起大落 |
| 日变化 | 正午抬、夜间落，相位合理 | 模型日变化几乎「照常上班」；实况主相窗畸形 |
| 拧 F10.7/Ap | 整图胖瘦平移或轻度变形 | 仍造不出观测上的分区锋面 |
| 残差 $|R|$ | 数 TECU 级常见，空间型偏缓 | 系统巨大；空间型跟着数据/指数窗口走 |
| 残差随旋钮 | 改 F10.7 → 残差整体滑 | 改模型开关时间戳 → 残差型不跟着走 |
| 叙事 | 「气候肖像在工作」 | 「达到天花板；残差度量偏离」≠「模型表征了精细暴」 |

**签名口诀（背）**：太漂亮、太光滑、换指数只整体胖瘦 → 气候；模型仍漂亮、实况已炸、拧旋钮造不出锋面 → 天花板。
...
**失败模式**：把 NRT 当 Final 写进精密暴例；用未映射 STEC 直接减模型 VTEC；数据侧时标是 GPS 周秒却按 UTC 切片——γ 假残差的高发入口。
...
**失败模式**：能编译但缺公共文件 → 全零/怪峰高；在线点选与本地包差一截却不记开关；NeQuick2 残差搬家评价 Galileo 官方改正——接口票据错。
...
**失败模式**：经度符号或地方时公式反号 → 日变化驼峰东西错位；积分上限一个含等离子体层、一个不含 → 全日同号数 TECU「假天气」。
...
**失败模式**：只贴残差彩图、不贴两侧绝对场 → 无法判断是天花板还是旋钮平移；色标不对称把 +5/−5 画成「一边倒风暴」。
...
**失败模式**：跳过 γ/β 直接宣 α；把「与 IRI 符合良好」写成「表征了该磁暴精细结构」；审稿人要观测时只能指模型图。
...
- 事件个例要写「该日该区 TEC 变化/精细结构」；  
- 需要小尺度梯度、闪烁或泡壁细节；  
- 强磁暴主相，且残差已显示天花板；  
- 审稿/验收明确要求观测产品溯源。

**选用失败模式（比对流程门口）：** 无 GNSS 却宣称「事件实况」；有 GIM 却只贴 IRI 色图当证据；Galileo 单频仿真却用 NeQuick2 残差评价官方改正。
...
**误解 11：峰密度 $N_mF2$ 大就等于 TEC 大，二者可互相替换写进摘要。**  
错。TEC 是整柱积分；「薄而密」与「厚而稀」可积出相近柱。摘要里应用哪写哪，禁止把峰密偷换成 TECU。

**误解 12：只要开了 IRI 暴期选项，残差变小就说明「模型抓住了这次风暴物理」。**  
错。统计滤镜可能让平均偏差变小，不等于复现本次正负相分区或锋面。残差变小仍须过 γ→β→α；天花板叙事优先于加冕。

**误解 13：F10.7 用「年均值」还是「当日值」无所谓，差几个 sfu 而已。**  
错。数 sfu 可对应数 TECU 的气候柱平移（见 §9.3 表）。对比实验必须记录用的是日值、多日平均还是哪个代理，并写进方法。
...
**题 15.** 均匀峰区片 $N_e=8\times10^{11}\,\mathrm{m^{-3}}$，厚 $50\,\mathrm{km}$，柱贡献约几 TECU？若另一片 $N_e=2\times10^{11}$、厚 $200\,\mathrm{km}$，柱贡献呢？说明什么？  
**答：** 前者 $8\times10^{11}\times5\times10^{4}=4\times10^{16}\,\mathrm{m^{-2}}=4\,\mathrm{TECU}$；后者同样 4 TECU。「薄而密」与「厚而稀」可积出相近柱——只比 TEC 看不见峰形。

**题 16.** 一眼区分：模型绝对场光滑、GIM 主相已炸、$|R|$ 巨大且拧 Ap 造不出锋面——气候正常还是天花板？正确叙事一句？  
**答：** 天花板。叙事：「气候模型达到天花板；残差度量偏离气候」——不是「IRI 表征了该暴精细结构」。

**题 17.** 调用清单 §13.3 的典型失败模式举两条。  
**答示例：** 经度/地方时反号导致驼峰东西错位；积分上限口径不一导致全日同号数 TECU 假天气。
...
**(5)** Galileo 单频：电文参数驱动 NeQuick-G 估得 $\widehat{\mathrm{STEC}}=20\,\mathrm{TECU}$。若频率约 $1.575\,\mathrm{GHz}$（L1 量级），用 §10.3 的课堂量级 $0.162\,\mathrm{m\,/\,TECU}$：估高 +5 TECU 时，延迟相对真值偏大约多少米？定性方向？  
**(6)**（签名题）给出「气候态」与「暴时天花板」各两条一眼判据（可抄 §12.0，须用人话）。
...
(5) $\Delta I\approx5\times0.162\approx0.81\,\mathrm{m}$（改正偏大、多减一截）；定性上延迟误差与 ΔSTEC 同号，按 $1/f^{2}$ 缩放。  
(6) 气候：大尺度光滑、换指数只整体胖瘦。天花板：模型仍漂亮而实况已炸；拧 Ap/F10.7 造不出观测锋面。

**加练：** 口述默写 §9.2 箭头链；手算题 (1) 换厚度 50 km、密度 $10^{12}\,\mathrm{m^{-3}}$ → 亦为 5 TECU——说明「薄而密」与「厚而稀」可积出相近柱，故只比 TEC 看不见峰形；再背误解 11–13 各一句。
...
**版本说明**：本文件按「机制 → 公式拆解 → 观测签名 → 分析步骤」与 01–03/05 课同深度加厚；解剖台统一「符号表 → 逐步推导 → 小数值例」；嵌入大气分层、Chapman/$N_e$ 廓线、日夜 TEC、EIA、指数时间线、延迟–频率、残差、IRI vs 测高仪等自制 CC0 图（图注含看什么/为什么/别误判）；引用项目名均来自 `PROJECTS.json` 核验。现象课 19–23 保持独立短文，不回灌成长文墙。
