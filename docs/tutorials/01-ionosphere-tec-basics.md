...
人话：$N_e$ 越高，$f_p$ 越高——雾越浓，介质特征振荡越快。数量级：$N_e\sim10^{11}$ m⁻³ 时 $f_p$ 约几 MHz 量级，远低于 GNSS 的 ~1.2–1.6 GHz。因此波能穿过（$f\gg f_p$），但折射率仍偏离 1，偏离量 ∝ $f_p^2/f^2$ ∝ $N_e/f^2$。
...
取 GPS L1 $f_1=1575.42\times10^6$ Hz，STEC = 10 TECU = $1.0\times10^{17}$ m⁻²：
...
**答：**
$$
\mathrm{STEC}=\int N_e\,\mathrm{d}s\quad(\mathrm{m}^{-2}),\qquad 1\,\mathrm{TECU}=10^{16}\,\mathrm{m}^{-2}.
$$
...
**答：**
$$
\cos30^\circ\approx0.866,\quad
R_E/(R_E+H)=6371/6821\approx0.934,\quad
\sin z'\approx0.809,\quad
\cos z'\approx0.588,\quad
M\approx1.701;
$$
STEC≈34.0 TECU。
...
**题 8.** $f_p\propto\sqrt{N_e}$。当 $N_e\to4N_e$（*f* 固定）时，*f_p* 与 $(n_g-1)$ 各如何变？  
**答：**
$$
f_p\to2f_p,\qquad n_g-1\propto N_e\to4\ \text{times}.
$$
...
| 符号 | 单位 | 角色 |
|---|---|---|
| $40.3$ | m³·s⁻² | 常数组合 $e^{2}/(8\pi^{2}\varepsilon_0 m_e)$ 的工程值 |
| $N_e$ | m⁻³ | 局地电子密度 |
| $f$ | Hz | 载波频率 |
...
**题 C3.** E=40°，$H=450\,\mathrm{km}$，$R_E=6371$：求 $M$；若 STEC=30 TECU，VTEC≈?  
**答：**
$$
\cos40^\circ\approx0.766,\quad R_E/(R_E+H)\approx0.934,\quad \sin z'\approx0.715,\quad \cos z'\approx0.699,\quad M\approx1.43;
$$
VTEC≈21.0 TECU。
