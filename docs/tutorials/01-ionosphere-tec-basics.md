...
其中系数 **40.3**（约 $40.3\,\mathrm{m}^{3}\mathrm{s}^{-2}$）来自 e^2/(8pi^2varepsilon_0 m_e) 等常数组合；当 *N_e* 用 m⁻³、*f* 用 Hz 时，40.3 N_e/f^{2} 无量纲。
...
取 GPS L1 $f_1=1575.42\times10^6\,\mathrm{Hz}$，STEC =10 TECU=1.0×10^17 m⁻²：
...
人话：横截面积 1 m² 的柱子里，电子按 10^{16} 一包数。32 TECU = $3.2\times10^{17}\,\mathrm{m}^{-2}$。
...
能默写并解释：$\omega_p$ 式、*n_p*/*n_g* 符号相反、I=40.3 STEC/f^2 每个符号、TECU 定义、M(E) 几何、一例 L1/L2 延迟数字、一例壳高敏感。不能只背「电离层有电子」。
