...
人话：*N_e* 越高，*f_p* 越高——雾越浓，介质特征振荡越快。数量级：$N_e\sim10^{11}\,\mathrm{m}^{-3} \text{ 时 *f_p* 约几 MHz 量级，远低于 GNSS 的 ~1.2–1.6 GHz。因此波能穿过（} f\gg f_p \text{），但折射率仍偏离 1，偏离量 } \propto f_p^2/f^2\propto N_e/f^2$。

**课堂追问**：若某处 *N_e* 突然高到 *f_p* 接近载波频率，会发生什么？——反射/截止风险上升（短波通信常见；GNSS 正常电离层通常仍 $f\gg f_p \text{）。这帮助你理解「为什么公式里是 } N_e/f^2$ 这种结构」。
...
其中系数 **40.3**（约 $40.3\,\mathrm{m}^{3}\mathrm{s}^{-2} \text{）来自 } e^{2}/(8\pi^{2}\varepsilon_0 m_e) \text{ 等常数组合；当 *N_e* 用 m⁻³、*f* 用 Hz 时，} 40.3 N_e/f^{2}$ 无量纲。
...
取 GPS L1 $f_1=1575.42\times10^6\,\mathrm{Hz} \text{，STEC } =10\,\mathrm{TECU}=1.0\times10^{17}\,\mathrm{m}^{-2}$：
...
人话：横截面积 $1\,\mathrm{m}^2 \text{ 的柱子里，电子按 } 10^{16} \text{ 一包数。32 TECU = } 3.2\times10^{17}\,\mathrm{m}^{-2}$。
...
- **A**：固定当地竖直雾厚 = VTEC。  
- **B**：卫星在天顶，$E=90^\circ ,\quad \cos E=0 ,\quad z'=0 ,\quad M=1$，STEC≈VTEC。  
...
**答：** $\mathrm{STEC}=\int N_e\,\mathrm{d}s$（m⁻²）；1 TECU = 10¹⁶ m⁻²。
...
1. *N_e*：电子数密度，单位 m⁻³。例：F2 峰白天可取 $N_e=1.0\times10^{12}\,\mathrm{m}^{-3}$（即 $10^{6}\,\mathrm{cm}^{-3}$）。  
...
| $N_e$ | m⁻³ | 局地电子密度 |
| $f$ | Hz | 载波频率 |
| $\mathrm{STEC}$ | m⁻² | $\int N_e\mathrm{d}s$ |
| $I$ | m | 一阶群延迟等效距离 |
...
能默写并解释：$\omega_p \text{ 式、*n_p*/*n_g* 符号相反、} I=40.3\,\mathrm{STEC}/f^{2} \text{ 每个符号、TECU 定义、} M(E)$ 几何、一例 L1/L2 延迟数字、一例壳高敏感。不能只背「电离层有电子」。
