...
先分清整柱、峰值、不规则与几何：VTEC 看大尺度，foF2/掩星看峰与高度，ROTI 看起伏，双频组合与映射函数说明 TEC 从何而来。

![分层 + Ne 剖面示意](./images/fig-ne-profile-layers.png)

> **看图要点**
> - D/E/F 沉积不同；VTEC 是整柱积分，F2 峰与高度需另眼。
> - 峰抬升/掏空看 foF2 或掩星，大尺度正负相看 VTEC。
> 图源：本仓库自制示意（非实测）。

![Chapman Ne 与分层对照](./images/fig-chapman-ne.png)

> **看图要点**
> - Chapman 峰密与分层对照：峰密度 ≠ 整柱，峰高/峰密变化也未必同比影响柱含量。
> - 读图先标明自己看的是「峰」还是「柱」。
> 图源：本仓库自制示意（非实测）。
...
> **看图要点**
> - ROTI 标 TEC 变化率起伏热点，日落后低纬常见羽/热点。
> - 先对 LT 与磁纬；热点不等于电子多，也不等于平均 TEC 高。
> 图源：本仓库自制示意（非实测）。

![双频 TEC 几何无关组合示意](./images/fig-dualfreq-tec.png)

> **看图要点**
> - 延迟 $\propto 1/f^2$；双频差分消掉几何，剩 STEC（加偏差）。
> - 读 TEC 前确认频率对、DCB 与模糊度处理口径（见 02）。
> 图源：本仓库自制示意（非实测）。

![STEC→VTEC 映射函数 / 薄壳示意](./images/fig-mapping-function.png)

> **看图要点**
> - 薄壳假设下 STEC $= M(e)\times$ VTEC；低仰角的 $M$ 与映射误差更大。
> - 组网/GIM 时声明壳高与截止高度角，并警惕 IPP 扫描假波（见 22）。
> 图源：本仓库自制示意（非实测）。
