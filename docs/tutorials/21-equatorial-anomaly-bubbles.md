...
> **看图要点**
> - **机制**：幅度闪烁指数 S4 由衍射/聚焦起伏定义；低频通常更敏感。
> - **观测签名**：L5/L2 常高于 L1；事件窗内抬过中等/强阈值。
> - **分析用法**：与 ROTI 同窗互证；谈定位可用性时看多频。
> 图源：本仓库自制示意（非实测）。

![相位闪烁示意](./images/fig-phase-scint.png)

> **看图要点**
> - **机制**：不规则使载波相位残差剧烈抖动；σφ 概括相位闪烁强度。
> - **观测签名**：短时相位毛刺爆发，滚动 σφ 抬升。
> - **分析用法**：与 S4/ROTI 对照；周跳与真闪烁要分开声明。
> 图源：本仓库自制示意（非实测）。
...
> **看图要点**
> - **机制**：EPB 耗空通道沿磁力线映射到南北低纬，形成可识别的闪烁/泡活动带。
> - **看什么**：带状区相对海岸与磁赤道的走向；与 ROTI 热点是否同窗同区。
