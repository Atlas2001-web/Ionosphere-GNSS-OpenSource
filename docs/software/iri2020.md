...
首跑把 CMake 输出和 **198 条** `Warning: Fortran 2018 deleted feature …` 全打到 **stdout**（本机 `build.log` 1108 行），可执行文件落在 `iri2020/src/iri2020/iri2020_driver`，每次运行 `cwd` 切到 `src/iri2020/data/` 读系数与指数。建议用 editable 安装：之后要换指数文件或改驱动都在这个源码树里改。
