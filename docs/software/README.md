...
本目录共有 **125 篇**操作手册（合计 **27365 行**，`wc -l`，不含本索引）：命令、输入输出、坑、选型。不是教材正文。
...
最近新增（用法讲解）：**gkit-bias**（2026-09-24 06:54 EDT；tip **`b8b7dd8`**/GPL-3.0/★**8**；`gkit_bias` **1357040** B；`-V` **v1.0.0 (commit b8b7dd8)**；无参 exit **2**；`*aether*.conf` 缺数据/`RINEX=0`/`no AETHER .amb`；CLK 未实现；**未臆造 OSB**；交叉 great-upd/great-ifcb/clkcomb/pride-pppar/data-access；下一优先 MCOSB 或 gps-sdr-sim）。
