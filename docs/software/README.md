...
**下一优先（质检二遍，按弱→强）：** 停写门槛已近——剩余主要是 **登记/环境受限**（`anubis` / `ionomoni` / `iono-scintillation` / `gfzrnx`）无本机真实 I/O 可补；**sh-gim** 保持边界。**已短硬入库 `pyglow` + `iri2016` + `apexpy` + `msise00`**。**已质检复跑** `gnss_lib_py` + `pyiri` + `pyirtam` + `fast` + `diffionmap` + `cddis-highrate-downloader`（stdout 对齐；CDDIS LIST 425 已记）。**勿抢** gps-measurement-tools / PyGPSClient（用法讲解下一组）。
**sh-gim：** 保持短边界，禁止注水扩写。  
