...
本目录共有 **74 篇**操作手册（合计 **17140 行**，`wc -l`，不含本索引）：命令、输入输出、坑、选型。不是教材正文。
...
最近新增（用法讲解）：**graphgnsslib**（`d861802`；georinex+RTKLIB 基线；无 ROS/Docker 未跑 FGO；交叉 rtklib/gnss_lib_py/great-pvt）+ **great-pvt**（v1.4.0/`8bd3d23`；GODN 1 h DF Float 120 历元 3D RMS≈0.006 m；交叉 pride-pppar/rtklib/ginan）。
