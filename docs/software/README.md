...
本目录共有 **41 篇**操作手册（合计 **9799** 行，`wc -l`，不含本索引）：命令、输入输出、坑、选型。不是教材正文。
...
**下一优先（质检二遍，按弱→强）：** 停写门槛已近——剩余主要是 **登记/环境受限**（`anubis` / `ionomoni` / `iono-scintillation` / `gfzrnx`）无本机真实 I/O 可补；**sh-gim** 保持边界。**已短硬入库 `pyglow` + `iri2016` + `apexpy` + `msise00`**。**用法讲解已入库** `gps-measurement-tools` + `pygpsclient` + `cycle-slip-correction` + `diffionmap` + `cddis-highrate-downloader`。**已质检复跑** `gps-measurement-tools` + `pygpsclient` + `gnss_lib_py` + `pyiri` + `pyirtam` + `fast` + `diffionmap` + `cddis-highrate-downloader`（stdout 对齐；CDDIS LIST 425 已记）。**质检新短硬** `geospacelab`（0.14.8；OMNI SYM_H=-234 + Madrigal TEC max=114）。**勿抢** `gdds` / `glab-upc` / `pynmeagps`（用法讲解）。
**sh-gim：** 保持短边界，禁止注水扩写。  
