...
本目录共有 **117 篇**操作手册（合计 **25779 行**，`wc -l`，不含本索引）：命令、输入输出、坑、选型。不是教材正文。
...
7. GIM 课 [ionex-gim](./ionex-gim.md) / Rust [ionex-rs](./ionex-rs.md)；SH-GIM 仅边界；CLAS → [claslib](./claslib.md)；MADOCA → [madocalib](./madocalib.md)；B2b → [b2blib](./b2blib.md)/[rtklib-b2b](./rtklib-b2b.md)/[navdecoder](./navdecoder.md)；多路径 QC → [gnss-multipath-analysis](./gnss-multipath-analysis.md)
