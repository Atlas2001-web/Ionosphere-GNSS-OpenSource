...
本目录共有 **18 篇**操作手册（合计 **5064** 行，`wc -l`，不含本索引）：命令、输入输出、坑、选型。不是教材正文。
...
**下一优先（质检二遍，按弱→强）：** 受限篇 guide 对齐（`anubis` / `ionomoni` / `iono-scintillation`；**禁臆造 stdout**）→ 可选二遍 `cssrlib` / `gnss-tec` / **pinot** / **autorino**。**R8 已完成** `pytecgg`（ABMF 全日 veq 实跑）+ `gfzrnx`（`-sifl` guide-only）。新缺篇 **pinot** / **autorino** 已短硬入库（用法讲解）。
**sh-gim：** 保持短边界，禁止注水扩写。  
...
| 14 | [cssrlib.md](./cssrlib.md) | Python PPP / PPP-RTK（CLAS/HAS/BDS） | 243 | **已短硬** · 本机 1.2.1 + cssrlib-data SPP 60 历元实跑（CLAS/HAS 未跑完） |
| 15 | [gnss-tec.md](./gnss-tec.md) | RINEX → 相对斜 TEC | 194 | **已短硬** · 本机 1.1.1 + `14601736.18o` 实跑 |
| 16 | [pinot.md](./pinot.md) | “not only TEQC” QC/预处理批壳 | 280 | **已短硬** · orderfile/sitecheck/metacheck 实跑；qualitycheck 无 Linux teqc |
| 17 | [autorino.md](./autorino.md) | 厂商 RAW 拉取 → RINEX3/4 | 233 | **已短硬** · 本机 2.4.2 cfgfile_check 实跑；厂商 convert 环境受限 |
