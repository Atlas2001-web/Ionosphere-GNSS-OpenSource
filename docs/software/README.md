...
本目录共有 **18 篇**操作手册（合计 **5068** 行，`wc -l`，不含本索引）：命令、输入输出、坑、选型。不是教材正文。
...
**下一优先（质检二遍，按弱→强）：** **pinot** / **autorino** → 受限篇 guide 对齐（`anubis` / `ionomoni` / `iono-scintillation`；**禁臆造 stdout**）。**R9 已完成** `cssrlib`（`nav.t` 坑+SPP 复跑）+ `gnss-tec`（stdout 对齐 + type N NAV 坑）。**R8** `pytecgg`/`gfzrnx`。**勿碰**同事新建中的 `rnxcmp.md` / `nequickg.md`。
**sh-gim：** 保持短边界，禁止注水扩写。  
