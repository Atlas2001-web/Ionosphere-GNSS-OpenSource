...
目录：[`PROJECTS.json` → `rinex2bin`](../../PROJECTS.json) · crates.io **`rinex2bin` 0.1.0**（bin **`rinex2bin`**）· 上游 <https://github.com/rtk-rs/rinex2bin>（GitHub **301→** [nav-solutions/rinex2bin](https://github.com/nav-solutions/rinex2bin)）· tip **`cc74460`**（tag **`v0.1.0`**；仓内仍 **0.1.0**）· **MPL-2.0** · ★**1** · 依赖锁 **`rinex` 0.17.0** + **`binex` 0.4.2** · 本机验证 **rustc 1.98.1** + `cargo install rinex2bin --locked`（2026-09-24 07:01 EDT；质检复跑 **07:06 EDT**：`--skip` ESBC MN → MonumentGeo×**3** + Eph×**2640**（GLO **510**/SBAS **2130**）；`.bin` **10821632** B（=**2642×4096**）；无 `--skip` → **NotEnoughBytes**；流尽后 `timeout --signal=KILL` exit **137**）：ESBC MN `--skip` → debug **MonumentGeo×3** + **EphemerisFrame×2640**（GLO **510**/SBAS **2130**）；落盘短名 **`ESBC1770.20N.bin`** **10821632 B**（=**2642×4096**；流尽后忙等须 `timeout`/`kill`）；GLO3 切片 R01 `clock_offset_s`=**6.355904042721×10⁻⁵**≡RINEX；无 `--skip` 大头注释 → **`NotEnoughBytes`**；OBS 仅头；`demo.10o` → **`datime parsing`**
