...
6. **训练第 2 个 batch 报 `RuntimeError: unable to allocate shared memory(shm) … No space left on device`**（实测，Docker 默认 `/dev/shm` 只有 64 MB） → DataLoader 的多个 worker 通过 /dev/shm 传张量。崩溃的进程还会在 /dev/shm 留下 `torch_*` 文件（本机残留 8 个，每个 9.6 MB）继续占用空间，所以紧接着改用 `set_sharing_strategy("file_system")` 重试也失败了（该策略同样使用 /dev/shm）。修复：先执行 `rm -f /dev/shm/torch_*`，再用 §4.2 的 `num_workers=0` 补丁重跑（或者启动容器时加 `--shm-size=2g`，本机未测）
