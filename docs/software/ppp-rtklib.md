...
目录：[`PROJECTS.json` → `ppp_rtklib`](../../PROJECTS.json) · 上游 <https://github.com/mulin33/ppp_rtklib> · **无 LICENSE 文件**（默认保留所有权利；源码头保留 Takasu RTKLIB 版权声明，原 RTKLIB 为 BSD-2-Clause）· tip **`c9f133c`**（2025-06-25）· ★4 · 内核 **RTKLIB 2.4.2**（`.pos` 头 `ver.2.4.2`）· 原生工程 **VS2015 `RTKLIB_242.vcxproj`**（Windows）· 本机：gcc **14.2.0** 改编译参数后 Linux 可编译 → 二进制 **238192** B；仓内 ALGO 2022-03-08 全天 → **2880** 历元全 Q=6，与仓内 Windows 结果 `result/ppp.pos` **逐行一致** · **2026-09-26 00:53–00:58 EDT** · **质检复跑**（2026-09-26 00:59–01:02 EDT，gcc 14.2.0）：tip `c9f133c`/无 LICENSE/`.c` **25** 个（去 `r_clk.c` 后 24）共 **12616** 行（含 .h）/克隆 **153 MB**；原样编译复现 `clutter.c: unknown type name 'DIR'` 与链接 `multiple definition of 'readrnxc'`；按 §3 四处处理后二进制 **238192** B；运行 exit 0、processing **2880**（Q=0×1+Q=6×2879）、第 2881 行“预处理错误!”；`.pos` 数据 **2880** 行与作者 `result/ppp.pos` **diff 0 行**（仅 4 行头部路径 `\`↔`/` 不同）、首/次/末历元逐字一致、`ppp.trace` **13688348** B、ns 5–10；§4.1 三种改法末历元 **206.9226/200.7503/208.4716** 与 apt `rnx2rtkp` 2.4.3 **201.0324** 逐字复现（IGS API llh `45.95580039898852 -78.07136759112825 200.8294485278988`）
