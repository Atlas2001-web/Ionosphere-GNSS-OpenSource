...
本机验证（**2026-09-26 00:31–00:40 EDT**）：Python **3.13.5** venv；numpy **2.5.3** / pandas **3.0.6** / scipy **1.18.1** / pyproj **3.8.0** / astropy **8.0.1** / gnsscal **1.1.2** / unlzw3 **0.2.3** / wget **3.2** / pygeodesy **26.9.9**。原样 `import CalcSwd` → **IndentationError**；打 3 处一行补丁后，GRAZ 站 2024-01-01 00 UTC 跑通 GPT3+GMF 与 VMF1 两条 SWD 路径（stdout 见 §4）。CDDIS 星历下载本机**失败**（FTPS `425 Security: Bad IP connecting.`），SINEX 写出整链**未跑**（见 §6 坑 8/9）。**质检复跑**（2026-09-26 00:46–00:55 EDT，同 venv 版本）：tip `3e4552b`/★10/forks 2/GPL-3.0/`.py` 共 **2061** 行、`gpt3_5.grd` 与 TU Wien `cmp` 相同；原样 `import CalcSwd` → **IndentationError**；按 §3 打 **1（CalcSwd）+3（VMF.py）** 处 sed 后 `git diff --stat` = `CalcSwd.py 2 +-`/`VMF.py 8 ++++----`、`import OK`（**只为 import 需 1 处；走 VMF1 路径才需另 3 处**）；§4a 全部 10 行、§4b 4 行、`CalcSTD` GMF `UnboundLocalError`/VMF `STD=[4.3127 22.0409] ZTD=[2.1639]`、4×`VMFG_20240101.H*` **668655** B 逐字复现；CDDIS FTPS `230 Login successful.` 后 `nlst` → **425 Security: Bad IP connecting.**、`igv` 返回 `./IGR22952.SP3` 但文件不存在、`'IGS'` → `UnboundLocalError: … 'destination'` 复现。墙时不可比：本机共享负载 load≈37/8 核，§4b 缓存命中（mtime 不变、不重下）时 `read_vmf1_grid` 单次仍 **2 m 49 s**（原稿 10.8 s 含下载）——慢在 CPU 争用，不是重下。
