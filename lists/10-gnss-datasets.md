...
科罗拉多大学 LASP 的太阳辐照数据中心，收录约 280 个数据集：Penticton F10.7（每日三测与近正午值）、复合 Lyman-α、FISM2 EUV 光谱、GOES XRS/EUVS、TIMED/SDO/TSIS 辐照等。LaTiS 接口在 URL 里选变量、时间范围和时间格式，输出 CSV/JSON/文本；另有只含 29 个数据集的 HAPI 2.0（仅 CSV）。两者都不用账号，适合给 IRI/NeQuick、热层模式和耀斑 TEC 事件配太阳输入。不是 GNSS 数据源。
