...
个人开发者的 Python 小工具，MIT 许可。按时间与 URSI 台站码从 GIRO DIDBase 按月拉 Digisonde 标定参数（foF2、hmF2、MUF(D)、B0、TEC、置信度 CS 等），缓存为 NetCDF 后返回 xarray Dataset。PyPI 最新版 2.1.0 仍调用已下线的 DIDBGetValues 接口（404），且漏声明 pytz、dask 依赖，需按本库手册改写到 fastchar/getbest 才能用。GIRO 数据为 CC BY-NC-SA 4.0，须致谢站点。
