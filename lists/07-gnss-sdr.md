...
用分立射频元件与 TinyFPGA 搭建偏「模拟前端」风格的 GPS 接收链路，帮助理解从射频到数据的路径。适合电子与射频教学、极客实验。调试门槛高、不是量产导航产品；算法研究更常见路径仍是 ADC 采样后接软件接收机。元器件公差与布局对前端噪声系数影响很大。焊接与屏蔽不良会直接抬高本底噪声。
...
从中频采样走完捕获—跟踪—导航的经典路径，教材配套友好。适合 SDR/接收机课程。现代多星座多频与实时性能请看 gnss-sdr/PocketSDR。
...
面向 B1C/B2a 信号结构的 MATLAB SDR 路径，便于读懂北斗新信号。适合课程实验。实时多通道与现场射频请看 gnss-sdr 等工程实现。
...
语言：Python · 许可：GPL-3.0 · 星标约：21 · 宿主：github
...
维护者提供的 Docker 镜像，预装 GNSS-SDR 及相关依赖，降低在本机编译 GNU Radio/GNSS-SDR 的门槛。适合课程实验与快速复现。镜像版本可能滞后于上游 GNSS-SDR 主线；生产或最新特性请按官方文档自行构建。
...
语言：— · 许可：proprietary (no code in repo; binary via Locus Lock, source licensing via UT Austin OTC) · 星标约：4 · 宿主：github

仓库仅说明，无代码（只有 README 与一张应用示意图；源码/二进制授权需经 UT Austin 技术商业化办公室或 Locus Lock）。README 介绍 UT Austin 无线电导航实验室开发 15 年以上的 C++ GNSS 软件接收机 GRID：SIMD 加速，支持多星座、多频、多天线，用于 RTK、电离层探测、欺骗检测和在轨运行。适合查阅其能力与相关论文；需要可直接编译的开源 SDR 时，请用 gnss-sdr、GNSS-SDRLIB、FGI-GSRx。
...
生成 Galileo E1B/C 基带信号流，可经 USRP（UHD）发射或写入文件，支持实时导航电文转发和键盘实时改位置，补齐 gps-sdr-sim 偏 GPS 的空白。适合 Galileo 基带与接收机试验，离线结果可用 GNSS-SDR 解算验证。仓库未声明许可；射频发射须遵守当地无线电法规。
...
面向微纳卫星/立方星工程的 GNSS 仿真相关 Python 工具，用于地面验证导航算法与接口。适合学生卫星与小型航天器软件队。通常不能替代机构级航天器动力学+GNSS 仿真器。轨道力学简化程度要在任务设计评审里说清楚。
...
语言：MATLAB · 许可：book companion · 星标约：— · 宿主：official_site

科罗拉多大学 CCAR 为《A Software-Defined GPS and Galileo Receiver》提供的支持主页，含常见问题与前端模块信息。配合 MATLAB SoftGNSS 学习从捕获、跟踪到导航电文。不再销售前端硬件；实践采集可改用其他 SDR 前端并对照社区 SoftGNSS 仓库。

#### [SoftGPS-CU-Boulder](https://www.colorado.edu/lab/rf-satnav/resources/original-open-source-gpsgnss-sdr)  
*🏷️ 高校实验室*

语言：MATLAB · 许可：book companion / GPL front-end bits · 星标约：— · 宿主：official_site
...
基于 Qt 的 GNSS-SDR 实时监视界面：通过 UDP 接收 GNSS-SDR 的 Monitor 与 PVT 两路序列化数据流，显示各通道 C/N0、多普勒、星座图，以及地图位置、高度和 DOP，可与接收机同机或异机运行。使用前须在 GNSS-SDR 配置文件里开启这两路输出；依赖 GNSS-SDR 主程序，2024 年后更新较少。
...
将多路径抑制（MMT）与直接定位估计结合的开源模块，面向 GPS L1 信号层定位与抗多路径研究。适合 SDR/基带算法试验。依赖 MATLAB；与完整软件接收机（如 GNSS-SDR、PocketSDR）相比，覆盖单频模块级能力，工程集成需自行对接前端。
...
OreSat 项目 GPS 接收机软件部分，与 oresat-gps-hardware 配套，覆盖星上接收相关流程与接口。适合立方星载荷软件与开源航天参考。面向特定硬件与任务约束，不是通用多星座精密定位或 RTK 引擎。星上算力与存储配额会限制可长期开启的日志等级。
